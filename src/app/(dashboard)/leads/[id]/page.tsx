...
export default async function LeadDetailPage({ params, searchParams }: LeadDetailPageProps) {
  ...
  const declaredServiceLabel = getDeclaredServiceLabel(lead);
  const { captureContext, customerDescription } = parseLeadDescription(lead.description);
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-muted-foreground">{lead.contact_name} quiere avanzar una oportunidad en {displayLocation}.</p>
        </div>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-4 md:grid-cols-5">
          <Card className="p-5">
            ...
            <p className="mt-3 text-2xl font-semibold">{formatEstimatedBudget(lead.estimated_budget)}</p>
          </Card>
          <Card className="p-5">
      ...
      <section className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 flex items-start gap-3 rounded-md bg-muted p-4">
            ...
            <p className="whitespace-pre-line text-sm leading-6">{customerDescription}</p>
          </div>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-md border border-border p-3">
              <p className="text-xs text-muted-foreground">Servicio declarado</p>
              <p className="mt-1 text-sm font-medium">{declaredServiceLabel}</p>
            </div>
            <div className="rounded-md border border-border p-3">
          ...
          <div className="mt-4 rounded-md border border-border p-3">
            <p className="text-xs text-muted-foreground">Rango declarado</p>
            <p className="mt-1 text-sm font-medium">{budgetRangeLabels[lead.budget_range]}</p>
          </div>
        </Card>
      </section>

      {captureContext.length ? (
        <section>
          <Card className="p-5">
            <h2 className="font-semibold">Contexto de captación</h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {captureContext.map((item) => (
